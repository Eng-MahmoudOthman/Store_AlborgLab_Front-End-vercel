...
export default function ItemConsumption({ele}) {
   ...
   return (
      <Fragment>
         <div  className={` container border border-2 rounded-2 my-3 p-1 `}>
                        ...
                        <tr>
                           <td colspan="4">
                              ...
                              <div className='d-flex justify-content-center align-items-center'>
                                 <button className='btn btn-sm bg-main opacity-75 px-3 my-2' onClick={()=>{setCountItem((prev)=> prev + 1)}}>+</button>
                                 ...
                                 <button className='btn btn-sm bg-main opacity-75  px-3 my-2' onClick={()=>{setCountItem((prev)=> prev <= 0? 0 : prev - 1)}}>-</button>
                              </div>
                              {isLoading ? 
                                    <button className="btn btn-sm bg-main w-75 p-0 my-1"> <Loading color="white"  width={20} height={20} strokeWidth="6"/></button>
                                 : 
                                    <button className={`btn btn-sm w-75 p-1 my-1 ${ele.expired_date < Date.now()?"btn-danger":"bg-main"}`} onClick={()=>{handleAddConsumption(ele._id , ele.item_quantity)}}>Consumped {ele.expired_date < Date.now()?<i className="fa-solid fa-hourglass-end"></i>:"🕗"} </button>
                              }
                           </td>
                        </tr>
                     </tfoot>
